...
if(isset($_GET["action"]))
{
	if($_GET["action"] == "delete")
	{
		...
		foreach($cart_data as $keys => $values)
		{
			if($cart_data[$keys]['item_id'] == $_GET["id"])
			{
				...
				$item_data = json_encode($cart_data);
				setcookie("shopping_cart", $item_data, time() + (86400 * 30));
				header("location:Sale.php?remove=1");
			}
		}
	}
	if($_GET["action"] == "clear")
	{
		...
		header("location:Sale.php?clearall=1");
	}
}
...
<div class="list-group" id="show-list">
		...
		<div class="container">
			<br />
			<h3 align="center">Birds products on Sale</h3><br />
			<br /><br />
			<?php
			$query = "SELECT * FROM tbl_sale ORDER BY id ASC";
			...
			<div class="col-md-3">
				<form method="post">
					<div style="border:1px solid #333; background-color:#f1f1f1; border-radius:5px; padding:16px;height:500px;" align="center">
						<img src="img/sale/<?php echo $row["image"]; ?>" class="img-responsive" /><br />
			...
			<div align="right">
				<a href="Sale.php?action=clear"><b>Clear Cart</b></a>
			</div>
			...
			?>
				<tr>
					...
					<td><a href="Sale.php?action=delete&id=<?php echo $values["item_id"]; ?>"><span class="text-danger">Remove</span></a></td>
				</tr>
			<?php	
